...
export default function ProfilePage() {
  ...
  // Ensure user is only set on client to avoid hydration mismatch
  useEffect(() => {
  ...
  // We now store photo directly as a Data URL inside editedProfile.profileImage for persistence.
  // objectPhotoUrl no longer needed for persistence; keeping in case of future optimizations.
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      const currentUser = typeof window !== 'undefined' ? (window.localStorage.getItem('currentUser') || (window.localStorage.getItem('user') && JSON.parse(window.localStorage.getItem('user') as string)?.email)) : null
      const email = typeof currentUser === 'string' ? currentUser : null
      const hasSupabase = !!process.env.NEXT_PUBLIC_SUPABASE_URL && !!process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY
      if (hasSupabase && email) {
        // Save main profile
        await fetch('/api/profile', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, profile: profileToSave }) })
        // Save children list (replace-all strategy)
        await fetch('/api/children', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, children: editedChildren }) })
        // Save notification settings
        await fetch('/api/notify', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, notify: notifications }) })
      } else if (typeof window !== 'undefined') {
        window.localStorage.setItem(PROFILE_KEY, JSON.stringify(profileToSave))
        window.localStorage.setItem(CHILDREN_KEY, JSON.stringify(editedChildren))
  ...
  useEffect(() => {
    ...
    try {
      ...
      const currentUser = window.localStorage.getItem('currentUser') || (window.localStorage.getItem('user') && JSON.parse(window.localStorage.getItem('user') as string)?.email)
      const email = typeof currentUser === 'string' ? currentUser : null
      if (hasSupabase && email) {
        Promise.all([
          fetch(`/api/profile?email=${encodeURIComponent(email)}`).then(r => r.json()).catch(() => ({})),
          fetch(`/api/children?email=${encodeURIComponent(email)}`).then(r => r.json()).catch(() => ({})),
          fetch(`/api/notify?email=${encodeURIComponent(email)}`).then(r => r.json()).catch(() => ({})),
          ...
          if (Array.isArray(children)) setEditedChildren(children.map((row: any) => row.data ?? row))
          ...
          if (!p) {
            ...
            if (storedProfile) try { setEditedProfile(JSON.parse(storedProfile)) } catch {}
          }
          if (!Array.isArray(children)) {
            const storedChildren = window.localStorage.getItem(CHILDREN_KEY)
            if (storedChildren) try { setEditedChildren(JSON.parse(storedChildren)) } catch {}
          }
          if (!notify) {
  ...
  const handleUploadDocs = (childId: number, files: FileList | null) => {
    ...
    setEditedChildren(editedChildren.map(c => c.id === childId ? { ...c, documents: [...c.documents, ...newDocs] } : c))
  }

  const handleDeleteDoc = (childId: number, docId: string) => {
    setEditedChildren(editedChildren.map(c => {
      if (c.id !== childId) return c
      const doc = c.documents.find((d:any)=> d.id===docId)
      if (doc?.objectUrl) URL.revokeObjectURL(doc.objectUrl)
      return { ...c, documents: c.documents.filter((d:any)=> d.id!==docId) }
    }))
  }
      ...
      <main className="container mx-auto px-4 py-6">
          ...
          <div className="fixed bottom-0 left-0 right-0 bg-white border-t shadow-sm py-4 px-4 flex justify-center z-40">
            {isEditing ? (
              <div className="flex gap-3 w-full max-w-md">
                <Button variant="outline" className="flex-1 text-blue-600 border-blue-300 hover:bg-blue-50" onClick={() => setIsEditing(false)}>
                  Cancel
                </Button>
                <Button className="flex-1 bg-primary hover:bg-primary/90 text-primary-foreground" onClick={handleSaveProfile}>
              ...
              <div className="flex gap-3 w-full max-w-md">
                ...
                <Button variant="outline" className="flex-1 text-blue-600 border-blue-300 hover:bg-blue-50" onClick={handleLogout}>Logout</Button>
              </div>
            )}
          </div>
        )}
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={(value) => setEditedProfile({ ...editedProfile, province: value })}
                        disabled={!isEditing}
                      >
                        <SelectTrigger>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setEditedProfile({ ...editedProfile, placeOfWork: e.target.value })}
                        disabled={!isEditing}
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setEditedProfile({ ...editedProfile, workAddress: e.target.value })}
                        disabled={!isEditing}
                      />
                    </div>
                  </div>
        ...
        <Card>
          ...
          <CardTitle>Emergency Contacts</CardTitle>
                  <CardDescription>Priority contacts in case of emergency</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setEditedProfile({ ...editedProfile, emergencyContact1Name: e.target.value })}
                        disabled={!isEditing}
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setEditedProfile({ ...editedProfile, emergencyContact1Phone: e.target.value })}
                        disabled={!isEditing}
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setEditedProfile({ ...editedProfile, emergencyContact2Name: e.target.value })}
                        disabled={!isEditing}
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setEditedProfile({ ...editedProfile, emergencyContact2Phone: e.target.value })}
                        disabled={!isEditing}
                      />
                    </div>
                  </div>
            ...
            <TabsContent value="children" className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-semibold text-blue-600">Child Profiles</h3>
                          ...
                          <div className="space-y-4">
                                ...
                                <div key={doc.id} className="flex items-center justify-between p-3 border rounded-lg hover:bg-muted/50">
                                  <div className="flex items-center space-x-3">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-sm text-muted-foreground">Uploaded {new Date(doc.uploadDate).toLocaleDateString()} • {doc.size}</p>
                                    </div>
                                  </div>
                                  <div className="flex items-center space-x-2">
                                    ...
                                    <Button variant="ghost" size="icon" onClick={() => handleDeleteDoc(child.id, doc.id)}>
                                      <Trash2 className="w-4 h-4 text-red-600" />
                                    </Button>
                                  </div>
                                </div>
                            ...
                            <div>
                              <input
                                ...
                                multiple
                                className="hidden"
                                onChange={(e)=> handleUploadDocs(child.id, e.target.files)}
                                disabled={!isEditing}
                              />
                              <Button
                                type="button"
                                ...
                                className="text-blue-600 border-blue-300 hover:bg-blue-50"
                                disabled={!isEditing}
                                onClick={() => document.getElementById(`upload-${child.id}`)?.click()}
                              >
                                <Upload className="w-4 h-4 mr-2" />
                  ...
                  <div>
                    ...
                    <div className="flex space-x-2 mt-2">
                      <Input
                        ...
                        className="flex-1"
                      />
                      <Button variant="outline" size="icon" className="text-blue-600 border-blue-300 hover:bg-blue-50" onClick={() => setShowPassword(!showPassword)}>
